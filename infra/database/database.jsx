...
const fallbackBd = 'developer';
function sanitizeSchema(schema) {
  const s = String(schema || fallbackBd).trim(); // fallback aqui
  if (!/^[a-zA-Z0-9_]+$/.test(s)) {
    console.warn('[db] SCHEMA inválido, usando "developer". Valor:', s);
    return fallbackBd;
  }
  return s;
}
...
export default async function getDb(schema) {
  ...
  const OPS = [
    { kind: 'DELETE_FROM', regex: new RegExp(`\\bDELETE\\s+FROM\\s+${PAT}`, 'gi') },
    { kind: 'INSERT_INTO', regex: new RegExp(`\\bINSERT\\s+INTO\\s+${PAT}`, 'gi') },
